...
module.exports.userCtrl = {
  register: async (req, res) => {
    ...
    const { code, message } = await userService.register({
      username,
      email,
      password,
    });
  ...
  verifyOtp: async (req, res) => {
    ...
    const { code, message, element } = await userService.verifyOTP({
      otp,
      email,
    });
    res.status(code).json({ message, element });
  },
  login: async (req, res) => {
    const { email, password } = req.body;
    const { code, message, user } = await userService.loginUser({
      email,
      password,
    });
    if (!user) {
  ...
  updateUser: async (req, res) => {
    ...
    if (newPassword) {
      if (newPassword.length < 8) {
        return res.status(400).json({ message: "Mật khẩu tối thiểu 8 kí tự" });
      } else {
        ...
        users[0]?.users?.length > 0
          ? {
              ...
              totalOrder: {
                ...
                totalPage: Math.ceil(
                  Number(users[0]?.meta[0]?.total) / Number(pageSize)
                ),
              },
            }
          : [],
