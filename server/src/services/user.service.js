...
module.exports.userService = {
  verifyOTP: async ({ otp, email }) => {
    try {
      ...
      const isValid = await otpService.validOtp({ otp, hashOtp: lastOtp.otp });
      if (!isValid) {
        return {
          code: 404,
  ...
  register: async ({ username, email, password }) => {
    ...
    console.log("otp", otp);
    sendOTP({ text: "Verify your email", email: email, otp: otp });
    return {
      code: 201,
  ...
  forgot: async ({ email }) => {
    ...
    sendOTP({ text: "Reset password", email, otp: otp });
    return {
      code: 201,
      element: await otpService.insertOtp({
