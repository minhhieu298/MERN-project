...
async function queryParams(query, req) {
  ...
  switch (query) {
    case "1":
      ...
      }).select("_id user paymentStatus orderItems totalAmount orderStatus").populate("orderItems.productId", "_id name image").sort("-createdAt");
      break;
    case "2":
      orders = await Order.find({
        user: req.user._id,
...
module.exports.orderCtrl = {
  ...
  getSingleOrder: async (req, res) => {
    ...
    }).populate("orderItems.productId", "_id name image").lean().exec((error, order) => {
      if (error) return res.status(400).json({
        message: error.message
      });
      if (order) {
  ...
  getOrdersAdmin: async (req, res) => {
    ...
    return res.status(200).json({
      data: orders[0]?.orders?.length > 0 ? {
        orders: orders[0]?.orders,
        total: {
          total: Number(orders[0]?.meta[0]?.total),
          page: Number(orders[0]?.meta[0]?.page),
          pageSize: Number(pageSize),
          totalPage: Math.ceil(Number(orders[0]?.meta[0]?.total) / Number(pageSize))
        }
      } : []
    });
  },
  ...
  getOrderDetail: async (req, res) => {
    const order = await Order.findById({
      _id: req.params.id
    }).populate("orderItems.productId addressId", "_id name image address district city state phone");
    if (!order) {
