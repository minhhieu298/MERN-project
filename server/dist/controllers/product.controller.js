...
module.exports.productCtrl = {
  ...
  getProducts: async (req, res) => {
    ...
    res.status(200).json({
      // products
      data: products[0]?.products?.length > 0 ? {
        products: products[0]?.products,
        meta: {
          total: Number(products[0]?.meta[0]?.total),
          page: Number(products[0]?.meta[0]?.page),
          pageSize: Number(pageSize),
          totalPage: Math.ceil(Number(products[0]?.meta[0]?.total) / Number(pageSize))
        }
      } : []
    });
  },
      ...
      if (!product) {
        ...
        Product.updateOne({
          ...
          $set: {
            ...
            price_after_discount: product.price * ((100 - Number(req.body.discount)) / 100)
          }
        }).exec((error, p) => {
          if (error) return res.status(400).json({
            message: error.message
